...
const PLUGIN_ROOT =
  "/Users/bregassa/Development/Project Nocra/lnreader-plugins";
...
class PluginManager {
  ...
  init() {
    ...
    const languages = fs.readdirSync(PLUGIN_PATH).filter((f) => {
      return (
        fs.statSync(path.join(PLUGIN_PATH, f)).isDirectory() && f !== "multisrc"
      );
    });
    ...
    for (const lang of languages) {
      ...
      const pluginFiles = fs
        ...
        .filter((f) => f.endsWith(".ts") && !f.includes(".broken."));
  ...
  getPluginByUrl(url) {
    for (const [id, pluginInfo] of this.plugins) {
      ...
      if (
        instance.site &&
        instance.site.length > 5 &&
        url.includes(instance.site)
      ) {
        return instance;
      }
      // Multi-domain support (like Syosetu)
      if (
        instance.novelPrefix &&
        instance.novelPrefix.length > 5 &&
        url.includes(instance.novelPrefix)
      ) {
        return instance;
      }
    }
